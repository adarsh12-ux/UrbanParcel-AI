-- Store user-supplied cadastral boundaries as real source data, separate from AI output.
...
ALTER TABLE public.parcels ADD CONSTRAINT parcels_source_check
  CHECK (source IN ('ai_extracted', 'official_cadastral', 'user_imported_cadastral', 'manual_edit', 'verified'));
...
BEGIN
  ...
  FOR feature IN SELECT value FROM jsonb_array_elements(p_geojson->'features') LOOP
    ...
    subdivision_number := NULLIF(BTRIM(COALESCE(feature_properties->>'subdivision_number', feature_properties->>'subdivision_no')), '');
    ...
    IF feature->>'type' <> 'Feature'
      ...
      RAISE EXCEPTION 'Every cadastral feature must be a Polygon or MultiPolygon with parcel_identifier';
    END IF;
    ...
    feature_properties := jsonb_set(feature_properties, '{source}', '"user_imported_cadastral"'::jsonb, true);
    feature_properties := jsonb_set(feature_properties, '{review_status}', '"needs_review"'::jsonb, true);

    INSERT INTO public.parcels (
      project_id, parcel_identifier, survey_number, subdivision_number, source_file,
      area_sqm, perimeter_m, source, review_status, attributes, geometry, source_job_id
    ) VALUES (
      p_project_id, parcel_identifier, survey_number, subdivision_number, source_file,
      ...
      'user_imported_cadastral', 'needs_review', feature_properties, geometry_value, NULL
    )
  ...
  RETURN jsonb_build_object('inserted', inserted_count, 'parcels', v_parcel_count, 'source', 'user_imported_cadastral');
END;
$$;
